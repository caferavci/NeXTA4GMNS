...
int g_ParserStringSequence(std::string str, std::vector<string> &vect)
{
...
double g_GetPoint2LineDistance(GDPoint pt, GDPoint FromPt, GDPoint ToPt, double UnitMile, bool requirement_for_full_intersection)
{
    float U;
	...
	if (requirement_for_full_intersection == true)
	{
...
int read_2_integers_from_a_string(CString str, long &value1, long &value2)
// read an integer from the current pointer of the file, skip all spaces
{
	value1 = 0;
	value2 = 0;
...
double  g_FindClosestYResolution(double Value, double x_coord, double y_coord)
{
	 vector<double> ResolutionVector;

	     double unit_y = 1.0 / 111.132;  // the equivalent lat per km
...
double  g_FindClosestXResolution(double Value, double x_coord, double y_coord)
{
	vector<double> ResolutionVector;
