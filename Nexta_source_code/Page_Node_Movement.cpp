...
IMPLEMENT_DYNAMIC(CPage_Node_Movement, CDialog)

CPage_Node_Movement::CPage_Node_Movement()
: CDialog(CPage_Node_Movement::IDD)
...
BEGIN_MESSAGE_MAP(CPage_Node_Movement, CDialog)
	ON_WM_LBUTTONDOWN()
	ON_WM_PAINT()
...
ON_BN_CLICKED(IDC_BUTTON_ExtendColumeWidth2, &CPage_Node_Movement::OnBnClickedButtonExtendcolumewidth2)
END_MESSAGE_MAP()
...
BOOL CPage_Node_Movement::OnInitDialog()
{
	...
	m_Column_names.push_back ("ib_node_id");  //0
	m_Column_names.push_back ("ob_node_id");   //1
	m_Column_names.push_back("ib_lane");   //2
	m_Column_names.push_back ("type"); //3
	m_Column_names.push_back ("Direction"); //4
	m_Column_names.push_back ("Prohibition"); //5
	m_Column_names.push_back ("volume"); //6
	m_Column_names.push_back ("Delay (sec)"); //7
	...
	for (unsigned int i=0;i< pNode->m_MovementVector .size();i++)
	{
		...
		str.Format("%d", movement.QEM_Lanes); // 2 nlanes
  ...
  if(bPhaseWindow == false || m_SelectedPhaseNumber == 1)
   pDC->TextOutA( PlotRect.CenterPoint().x-5, PlotRect.CenterPoint().y-5,str);
	...
	for (unsigned int i=0;i< pNode->m_MovementVector .size();i++)
	{
		...
		{
		bMovementIncluded = m_pDoc->IfMovementIncludedInPhase(pNode->m_NodeID, m_SelectedPhaseNumber, movement.in_link_from_node_id,movement.out_link_to_node_id );
		}
...
void CPage_Node_Movement::OnBnClickedButtonExtendcolumewidth2()
{
	SaveData();
	CDialog::OnCancel();
}
