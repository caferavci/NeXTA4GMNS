...
void CNetworkDataTabDlg::OnLvnItemchangedList(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	*pResult = 0;
	...
	while (pos != NULL)
	{
		...
		///////////////////////////////////////////////
		if (TabText.Find("Link") >= 0 || TabText.Find("Agent") >= 0)
		{
		long from_node_id = -1;
		long to_node_id = -1;
...
///////////////////////////////////////////////
		...
		for (int i=0;i< m_ListCtrl.GetColumnCount() ;i++)
		{
			...
			{   
				...
				m_pDoc->m_SelectedZoneID = zone_id;
		...
		if (TabText.Find("Demand") >= 0)
		{
			int zone_id = -1;

			for (int i = 0; i < m_ListCtrl.GetColumnCount(); i++)
			{
				if (strcmp(names[i].c_str(), "o_zone_id") == 0)
				{
					str = m_ListCtrl.GetItemText(nSelectedRow, i);
					zone_id = atoi(str);

					m_pDoc->m_SelectedOZoneID = zone_id;

				}
				if (strcmp(names[i].c_str(), "d_zone_id") == 0)
				{
					str = m_ListCtrl.GetItemText(nSelectedRow, i);
					zone_id = atoi(str);

					m_pDoc->m_SelectedDZoneID = zone_id;
